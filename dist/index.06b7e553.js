$(function() {
    ...
    if (isMobile) $("nav").addClass("fixed");
    else $("nav").addClass("desk");
    // NAV POSITION
    ...
    $(window).on("scroll", function() {
        ...
        if (!isMobile) {
            if (pos >= navPos + $("nav").height() && lastPos < pos) $("nav").addClass("fixed");
        ...
        if (pos2 > $("#contact").offset().top || pos + $(window).height() === $(document).height()) highlightLink("contact");
    ...
    function onScrollInit(items, elemTrigger) {
        ...
        items.each(function() {
            var elem = $(this), animationClass = elem.attr("data-animation"), animationDelay = elem.attr("data-delay");
            elem.css({
    ...
    $("#contact-form").submit(function(e) {
        e.preventDefault();
        $.ajax({
            url: "https://formspree.io/exe.massimelli@gmail.com",
            method: "POST",
            data: {
